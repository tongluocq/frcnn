...
parser.add_option("--config_filename", dest="config_filename",
                  ...
                  default="./tmp9_config.pickle")
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='./tmp9_model_frcnn.hdf5')
...
elif options.network == 'vgg_my':
    from keras_frcnn import vgg_my as nn
    C.network = 'vgg_my'
elif options.network == 'nasnet_my':
    from keras_frcnn import nasnet_my as nn
    C.network = 'nasnet_my'
    C.im_size = 331
elif options.network == 'mobnasnet_my':
    from keras_frcnn import mobnasnet_my as nn
    C.network = 'mobnasnet_my'
    C.im_size = 331
...
print('Starting training with verbose or not', C.verbose)
# vis = True
...
for epoch_num in range(num_epochs):
    ...
    while True:
        ...
        #print(" epoch_length ", epoch_length,  " rpn_accuracy_rpn_monitor",  len(rpn_accuracy_rpn_monitor))
        ...
        #print("  batch ", X.shape, Y[0].shape, len(Y), type(Y), type(img_data)  )
        ...
        #print(" pos neg sample length: ", len(neg_samples), len(pos_samples))
        ...
        print("rpn acc: ", sum(rpn_accuracy_rpn_monitor), sum(rpn_accuracy_for_epoch))
        ...
        progbar.update(iter_num,  [('epoch ', epoch_num + 1), ('rpn_cls', np.mean(losses[:iter_num, 0])), ('rpn_regr', np.mean(losses[:iter_num, 1])),
        ...
        if iter_num == epoch_length:
            ...
            print("\n")
            print("  rpn_accuracy_for_epoch  sum",  sum(rpn_accuracy_for_epoch), " len of rpn_accuracy_for_epoch", len(rpn_accuracy_for_epoch))
            print("\n")
